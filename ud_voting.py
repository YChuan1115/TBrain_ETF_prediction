...
def load_stock_ud(fname, workday):
    ...
    for day in workday:
        ...
        if ud_df[row_index] == 1:
            ud_data.append([1, 0, 0])

        elif ud_df[row_index] == -1:
            ud_data.append([0, 1, 0])

        else:
            ud_data.append([0, 0, 1])

    return ud_data


def load_day_ud(fname):
...
def main():
    ...
    score = []
    feature = []
    label = load_day_ud('day_ud_label.csv')
    ...
    for day in range(len(workday)):
        day_feature = []
        for stk in stk_name:
            day_feature.extend(stk_ud[stk][day])
        day_feature = np.asarray(day_feature)
        feature.append(day_feature)
