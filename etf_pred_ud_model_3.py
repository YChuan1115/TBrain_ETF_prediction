...
def get_batch_label(fn):
    title = [fn + '_收盤價(元)']
    df_value = pd.read_csv('stock_label/day_ud_label.csv', encoding='utf-8')
    workday_index = list(df_value['date'])
    workday_df = pd.read_csv('stock_workday/' + fn + '_workday.csv', encoding='utf-8')
    workday_df = list(workday_df['date'])
    ud_label = []
    for day in workday_df:
        row = workday_index.index(int(day))
        if df_value['ud'][row] == 1:
            ud_label.append([1, 0, 0])
        elif df_value['ud'][row] == -1:
...
def main():
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement = True, allow_soft_placement = True)) as sess:
        ...
        saver.save(sess, 'etf_ud_net_2/'+ fname + '_net/' + fname + '.ckpt')
